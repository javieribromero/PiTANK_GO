...
int ConfiguraSistema (TipoSistema *p_sistema) {
	...
	wiringPiISR(IR_RX_PIN, INT_EDGE_RISING, ir_rx_isr);
...
int main ()
{
	...
	fsm_t* keyboard = fsm_new(KEY_COL_1, keyboard_tt, &(sistema));
	fsm_t* player_fsm = fsm_new (WAIT_START, reproductor, &(sistema.player));

	fsm_t* torreta_fsm = fsm_new(WAIT_START, torreta_tt, &(sistema.torreta));
	next = millis();
	while (1) {
		fsm_fire (player_fsm);
		fsm_fire (keyboard);
